...
class NavPoint {
  ...
  static void linkSiblings(List<NavPoint> points, {int level = 0, NavPoint? parent = null}) {
  ...
  static NavPoint fromHtmlLi(String baseDir, dom.Element el) {
    ...
    final String? label = (a?.text ?? span?.text)?.trim();
  ...
  static List<NavPoint> htmlOlToList(String baseDir, dom.Element? el) =>
      el == null ? [] : el.children.map((li) => NavPoint.fromHtmlLi(baseDir, li)).toList();
  ...
  static NavPoint fromXmlElement(String baseDir, xml.XmlElement el) => NavPoint(
      ...
      labels: List.from(el.findElements('navLabel').map((el) => NavLabel.fromXmlElement(el))),
      content: pathJoin([baseDir, el.findElements('content').first.getAttribute('src')]),
      children: List.from(el.findElements('navPoint').map((el) => fromXmlElement(baseDir, el))));
  ...
  NavPoint.fromJson(Map<String, dynamic> json)
      : id = json['id'],
        ...
        klass = json['klass'],
        labels = (json['labels'] as List).map((j) => NavLabel.fromJson(j)).toList(),
        children = (json['children'] as List).map((j) => NavPoint.fromJson(j)).toList();
}

/// Represents Nav
class EpubNav extends _EpubXmlBase {
  EpubNav._(this._type, String basepath) : _basepath = basepath.replaceAll('\\', '/');
  ...
  void _loadFromJson(Map<String, dynamic> json) {
    ...
    _navMapList.addAll((json['navMapList'] as List).map((j) => NavPoint.fromJson(j)));
    linkPoints();
  }
  ...
  static EpubNav? fromNav(String dirname, String? xmlStr) {
    ...
    nav._navMapList.addAll(NavPoint.htmlOlToList(nav._basepath, root.querySelector('nav > ol')));
    nav.linkPoints();
    return nav;
  }
  ...
  static EpubNav? fromNcx(String dirname, String? xmlStr) {
    ...
    // loadNavMapList
    final navMapList = root.findElements('navMap');
    if (navMapList.isNotEmpty) {
      nav._navMapList
          .addAll(navMapList.first.findElements('navPoint').map((el) => NavPoint.fromXmlElement(nav._basepath, el)));
    }
